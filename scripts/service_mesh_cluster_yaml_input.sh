...
if ls | grep -q $CLUSTER_NAME.yaml; then
  ...
  if [ $ACTION = "rm" ]; then 
    ...
    awk '/values: \|/{print $0; exit} {print}' $CLUSTER_NAME.yaml values_part.yaml > header_part.yaml
    cp header_part.yaml updated.yaml
    cat $CLUSTER_NAME.yaml | yq '.values' | yq 'del(.istio_enabled) | del(.certManager) | del(.certManagerForISTIO) | del(.istiod) | del(.istio_enabled) | del(.istioIngressGateway) | del(.gateway)' | sed 's/^/  /' >> updated.yaml
    cat updated.yaml > $CLUSTER_NAME.yaml
